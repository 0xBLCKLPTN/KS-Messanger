defmodule KsMessanger.ChatsTest do
  use KsMessanger.DataCase

  alias KsMessanger.Chats
  ...
  describe "chats" do
    ...
    test "create_chat/1 with valid data creates a chat" do
      valid_attrs = %{chat_name: "some chat_name", members: ["option1", "option2"]}

      assert {:ok, %Chat{} = chat} = Chats.create_chat(valid_attrs)
      assert chat.chat_name == "some chat_name"
      assert chat.members == ["option1", "option2"]
    end
